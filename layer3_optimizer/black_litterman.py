...
Pipeline:
  ...
  3. BL posterior μ_BL = [(τΣ)⁻¹ + Ω⁻¹]⁻¹[(τΣ)⁻¹Π + Ω⁻¹Q]   Ω = τΣ
...
def _bl_posterior(Pi: np.ndarray, Sigma_annual: np.ndarray,
    ...
    """BL posterior with P=I, Ω=τΣ (equal weighting on prior and views)."""
    tau_S     = TAU * Sigma_annual
    inv_tauS  = np.linalg.inv(tau_S)
    inv_Omega = np.linalg.inv(tau_S)   # Ω = τΣ → inv same structure
