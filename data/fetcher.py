...
def _safe_download(tickers: list, start: str, end: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.warning(f"Download failed for {tickers}: {exc}")
        return pd.DataFrame()
...
def fetch_data(force_refresh: bool = False) -> pd.DataFrame:
    ...
    for market, ticker in {k: v for k, v in MARKET_TICKERS.items() if k != "US"}.items():
        ...
        for candidate in candidates:
            try:
                ...
                df[f"ret_mkt_{market}"] = (
                    mkt_ret[market].reindex(df.index, method="ffill").fillna(0.0)
                )
                df[f"price_mkt_{market}"] = mkt_raw[market].reindex(df.index, method="ffill")
                if candidate != ticker:
                    print(
                        f"  Benchmark {market}: index {ticker} failed, using ETF proxy {candidate}."
                    )
                loaded = True
                break
            except Exception:
        ...
        if not loaded:
            logger.warning(
                f"Benchmark {market} unavailable (tried {candidates}). Using SP500 proxy."
            )
    ...
    try:
        ...
        if not fx_raw.empty:
            ...
            for pair in FX_TICKERS:
                df[f"fx_{pair}"] = (
                    fx_raw[pair].reindex(df.index, method="ffill")
                    if pair in fx_raw.columns
                    else np.nan
                )
        else:
            for pair in FX_TICKERS:
        ...
        logger.warning(f"FX data unavailable: {exc}")
        for pair in FX_TICKERS:
            df[f"fx_{pair}"] = np.nan

    # ── FRED macro ─────────────────────────────────────────────────────────────
    if not FRED_API_KEY:
        logger.warning("FRED_API_KEY not set — macro features will be NaN.")
    for col_name, series_id in FRED_SERIES.items():
        s = (
            _fetch_fred_series(series_id, start_str, end_str)
            if FRED_API_KEY
    ...
    logger.info(f"Data fetched: {df.shape[0]} trading days, {df.shape[1]} columns.")
    return df
