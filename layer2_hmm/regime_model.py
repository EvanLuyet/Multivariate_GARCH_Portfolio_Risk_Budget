...
def _build_daily_features(daily_garch_vols: pd.DataFrame, data: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for col in ["vix", "yield_curve", "credit_spread", "usd_index"]:
        feat[col] = data[col].reindex(feat.index, method="ffill") if col in data.columns else 0.0
...
def _build_market_features(
    ret: pd.Series, price: pd.Series, us_ret: pd.Series | None = None
) -> pd.DataFrame:
    """
    ...
    if us_ret is not None:
        ...
        feat["rel_mom_21d"] = ret.rolling(21).sum() - us_ret.reindex(ret.index).rolling(21).sum()
    else:
        feat["rel_vol"] = 1.0
...
def _sort_and_floor(model: GaussianHMM, X_raw: np.ndarray, X_scaled: np.ndarray, vol_col: int):
    """
    ...
    state_means = np.array(
        [
            X_raw[raw_states == s, vol_col].mean() if (raw_states == s).any() else 0.0
...
def run_hmm(garch_history: dict, data: pd.DataFrame) -> dict:
    ...
    cache_file = cache_dir / f'hmm_{CONFIG_HASH}_{datetime.today().strftime("%Y%m%d")}.joblib'
    ...
    logger.info(
        f"Layer 2 — HMM training on {n_obs} daily observations "
    ...
    if cache_file.exists():
        try:
            ...
            if model.means_.shape[1] == n_feat:
                logger.info("Layer 2 — Loading HMM from cache …")
                cache_valid = True
            else:
                logger.info(
                    f"Layer 2 — Cache feature mismatch "
    ...
    probs_df = pd.DataFrame(
        probs, index=feat_df.index, columns=[f"p{i}" for i in range(HMM_STATES)]
    )
    ...
    logger.info(
        "Layer 2 — Regime distribution: "
        + "  ".join(
            f"{REGIME_LABELS[i]}: {counts.get(i, 0)} days " f"({counts.get(i, 0)/n_obs:.0%})"
...
def run_market_regimes(data: pd.DataFrame) -> dict:
    ...
    cache_dir.mkdir(exist_ok=True)
    cache_file = (
        cache_dir / f'mkt_regimes_{CONFIG_HASH}_{datetime.today().strftime("%Y%m%d")}.joblib'
    )

    if cache_file.exists():
        logger.info("Layer 2 — Loading market regimes from cache …")
        return joblib.load(cache_file)

    logger.info("Layer 2 — Fitting per-market regime HMMs (US / EU / Swiss) …")
    ...
    for market in MARKET_TICKERS:
        ...
        if market != "US" and ret.equals(us_ret):
            using_proxy = True
            print(
                f"  ⚠  {market}: benchmark download failed — regime will mirror US. "
        ...
        probs_series = pd.DataFrame(probs, index=feat.index, columns=["p0", "p1", "p2"])
        ...
        print(
            f"  {market:6s}: {REGIME_LABELS[current_regime]}{proxy_note}  "
...
if __name__ == "__main__":
    ...
    for market, info in mkt.items():
        probs_str = [f"{p:.0%}" for p in info["regime_probs"]]
        print(f'  {market}: {info["regime_label"]}  probs={probs_str}')
