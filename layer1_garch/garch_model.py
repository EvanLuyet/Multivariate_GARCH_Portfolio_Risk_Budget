"""
layer1_garch/garch_model.py — GJR-GARCH volatility forecasts + DCC-style covariance approximation.
...
The conditional covariance matrix is built via a DCC-style approximation
(not a full DCC-GARCH model):
...
def _build_cov(daily_sigmas: np.ndarray, resid_df: pd.DataFrame):
    ...
    # Guard: replace NaN from zero-variance columns (constant residuals) with
    # identity-like values before eigendecomposition
...
def run_garch(data: pd.DataFrame, weights: dict) -> dict:
    """
    Rolling GJR-GARCH + DCC-style covariance approximation at every quarter-end in the data.
    ...
    At each quarter-end date t we:
      ...
      3. Build the DCC-style covariance from the CORR_WINDOW trailing realized
         correlation of z_t.
    ...
    cache_file = cache_dir / f'garch_{CONFIG_HASH}_{datetime.today().strftime("%Y%m%d")}.joblib'

    if cache_file.exists():
        try:
            cached = joblib.load(cache_file)
            logger.info("Layer 1 — Loading GARCH history from cache …")
            return cached
        except Exception:
            cache_file.unlink()
    ...
    history = {}
    logger.info("Layer 1 — Running rolling GJR-GARCH …")
...
def get_daily_garch_vols(data: pd.DataFrame) -> pd.DataFrame:
    ...
    cache_dir.mkdir(exist_ok=True)
    cache_file = (
        cache_dir / f'daily_garch_vols_{CONFIG_HASH}_{datetime.today().strftime("%Y%m%d")}.joblib'
    )

    if cache_file.exists():
        return joblib.load(cache_file)

    logger.info("Layer 1 — Fitting full-sample GJR-GARCH for daily vol series …")
    result = {}
    for asset in ASSETS:
