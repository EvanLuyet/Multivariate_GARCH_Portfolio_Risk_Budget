...
def _build_strategy_returns(garch_history: dict, bl_history: dict,
                             data: pd.DataFrame,
                             weights_override: dict = None) -> pd.Series:
    """
...
def print_report(garch_current: dict, hmm_current: dict,
                 ...
                 fx_result: dict,
                 stock_picks: list,
                 ml_current: dict, ml_next_q: dict,
                 bl_current: dict, cost_result: dict,
                 current_weights: dict, final_weights: dict,
                 strat_ret: pd.Series, bench_ret: pd.Series,
                 current_w_ret: pd.Series) -> None:
    ...
    for market, info in market_regimes.items():
        ...
        print(f'  {market:6s}: {info["regime_label"]:18s} '
              f'Bull {probs[0]:.0%} / Trans {probs[1]:.0%} / Crisis {probs[2]:.0%}  '
              f'{trend_sym} {info["trend"]}')

    port_vol = garch_current['port_vol']
    vol_sig  = 'BUY ↑ add equity' if port_vol < VOL_LOW else \
               'SELL ↓ reduce equity' if port_vol > VOL_HIGH else 'HOLD — within target'
    ...
    for p in stock_picks:
        if p['ticker'] == 'N/A':
            print(f'    {p["note"]}')
            break
        print(f'    {p["rank"]}. {p["ticker"]:8s} {p["sector"]:22s} '
              f'3m score {p["mom_3m_pct"]:+.1f}z  Sharpe {p["sharpe_z"]:+.2f}  '
              f'— {p["note"]}')
    ...
    print(f'  Current weights:   '
          + '  '.join(f'{a} {current_weights.get(a, 0):.0%}' for a in ASSETS))
    print(f'  Base weights:      '
          + '  '.join(f'{a} {v:.0%}' for a, v in
                      __import__('config').BASE_WEIGHTS.items()))
    print(f'  Scenario A (Rebalance):')
    print(f'    Proposed:        '
          + '  '.join(f'{a} {w_opt[a]:.0%}' for a in ASSETS))
    print(f'    Change:          '
    ...
    if sb and sb['new_capital_usd'] > 0:
        ...
        print(f'    Result weights:  '
              + '  '.join(f'{a} {rw.get(a, 0):.0%}' for a in ASSETS))
        print(f'    Buys:')
        for a, info in sb['buys'].items():
            if info['amount'] > 0:
                print(f'      {a}: ${info["amount"]:,.0f} ({info["shares"]:.2f} shares)  '
    ...
    print(f'  Band breaches:     {", ".join(drifted) if drifted else "None (no asset drifted >5%)"}')
    ...
    col_w = 12
    print(f'  {"":22}  {"Base (65/20/15)":>{col_w}}  {"Current Wts":>{col_w}}  {"Proposed":>{col_w}}')
    ...
    print(f'  Risk Contributions: '
          + '  '.join(f'{a} {rc[a]:.0%}' for a in ASSETS))
...
def save_dashboard(garch_history: dict, hmm_result: dict,
                   ...
                   market_regimes: dict, fx_result: dict,
                   strat_ret: pd.Series, bench_ret: pd.Series,
                   current_w_ret: pd.Series) -> str:
    ...
    for i, market in enumerate(markets):
        ...
        for j, (p, c) in enumerate(zip(probs, colors)):
            ...
            if p > 0.1:
                ax.text(left + p / 2, i, f'{p:.0%}', ha='center', va='center',
                        fontsize=8, fontweight='bold', color='white')
            left += p
        ax.text(1.02, i, info['regime_label'], va='center', fontsize=9,
    ...
    patches = [mpatches.Patch(color=c, label=l)
               for c, l in [('#2ecc71', 'Bull'), ('#f39c12', 'Transition'), ('#e74c3c', 'Crisis')]]
    ...
    if not regime_s.empty:
        ...
        for i in range(len(rs) - 1):
            ...
            ax.axvspan(t0, t1, alpha=0.3,
                       color=REGIME_COLORS.get(int(rs.iloc[i]), '#999'), lw=0)
        ...
        for i, (lbl, col) in enumerate(
            ...
            if f'p{i}' in probs_df.columns:
                ax.plot([pd.Timestamp(d) for d in probs_df.index],
                        probs_df[f'p{i}'].values, color=col, lw=1.4, label=lbl)
    ...
    if not feat_imp.empty:
        ...
        ax.barh(range(len(top10)), top10.values * 100, color='#3498db', alpha=0.8, edgecolor='white')
