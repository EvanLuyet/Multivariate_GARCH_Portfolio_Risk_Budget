...
def compute_costs(
    ...
    if new_capital_usd > 0:
        ...
        scenario_b = dict(
            ...
            resulting_weights={
                a: round(float((w_old[i] * portfolio_value_usd + buys[i]) / total_after), 4)
...
if __name__ == "__main__":
    ...
    print(
        f"  Total cost    : ${res['scenario_a']['total_cost_usd']:.2f}  ({res['cost_bps']:.2f} bps)"
    )
    ...
    if res["scenario_b"]:
        ...
        for asset, info in res["scenario_b"]["buys"].items():
            if info["amount"] > 0:
                print(
                    f"  Buy {asset}: ${info['amount']:,.2f} ({info['shares']:.2f} shares)  "
