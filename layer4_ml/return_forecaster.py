...
def _build_feature_row(qe, garch_info: dict, hmm_info: dict, data: pd.DataFrame) -> dict:
    row = {}
        ...
        for asset in ASSETS:
            ...
            for days, lbl in [(21, "1m"), (63, "3m"), (126, "6m")]:
                ...
                if key in FEATURE_COLS:
                    row[key] = float(
                        data[col].iloc[max(0, loc - days) : loc].sum() if loc >= days else 0.0
                    )
...
def _compute_target(qe, next_qe, data: pd.DataFrame) -> dict:
    ...
    period = data[mask]
    return {a: float(period[f"ret_{a}"].sum()) if not period.empty else np.nan for a in ASSETS}
...
def _point_and_bounds(X_train, y_train, X_pred, valid_mask):
    ...
    # Sort to prevent quantile crossing (LGB models are independent — crossing is possible)
    q25, q50, q75 = sorted([q25, q50, q75])
    return xgb_mean, q25, q50, q75


def run_forecaster(garch_history: dict, hmm_history: dict, data: pd.DataFrame) -> dict:
    """
    ...
    cache_file = cache_dir / f'ml_{CONFIG_HASH}_{datetime.today().strftime("%Y%m%d")}.joblib'
    ...
    if cache_file.exists():
        ...
        if REQUIRED_KEYS.issubset(cached.keys()):
            logger.info("Layer 4 — Loading ML forecasts from cache …")
            return cached
        logger.info("Layer 4 — Stale cache (missing keys), re-running forecaster …")
        cache_file.unlink()

    logger.info("Layer 4 — Running walk-forward ML forecaster …")
    ...
    for asset in ASSETS:
        ...
        if v2.sum() >= MIN_TRAIN_QTRS // 2:
            ...
            current_2q[asset] = dict(
                point=m * 0.7, bl_point=m * 0.7, low=m * 0.7 - 0.04, high=m * 0.7 + 0.04
            )
